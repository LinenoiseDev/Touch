...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("touchMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("touchMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("touchMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("touch-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running touchMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
